...
Parser.prototype.addSourceComments = function(ast) {
  var filename = this.opts.filename;
  if (!ast || !filename) return ast;
  ...
  return [{
    ...
    props: {
      location: {
        expression: 'process.env.NODE_ENV !== "production" ? ' + JSON.stringify(filename) + ' : undefined'
      },
      props: {
        expression: 'props'
      }
...
Parser.prototype.toAst = function(test) {
  ...
  var ast = this.compiler.process(post(css), {from: filename});

  var transformed = this.parse(ast.root);
  return this.addSourceComments(transformed);
};
...
function replaceUrl(value) {
  ...
  tokens.nodes.forEach(function(value) {
    value.nodes.forEach(function(item) {
      if (item.type != 'url' || /^#/.test(item.url)) return;
      var url = item.url;
      if (!loaderUtils.isUrlRequest(url) && url.charAt(0) !== '~') return;
      if (!~['.', '/'].indexOf(url.charAt(0))) {
